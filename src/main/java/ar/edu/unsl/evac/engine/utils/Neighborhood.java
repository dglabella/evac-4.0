...
public interface Neighborhood {
    /***
     * Generates a set of (x,y) coordinates for specify the Moore neighborhood of the current (i,j)
     * cell in the cellular automaton (bi-dimensional).
     * 
     * @param i The i coordinate of the cell.
     * @param j The j coordinate of the cell.
     * @param sizeFactor Scales the Moore neighborhood making it bigger. This number has to be an
     *        integer greater or equal than 1 but not greater than floor((min(width,height)-1) / 2).
     ...
     * @return A list of arrays of size 2 which one containing (x,y) coordinates of the Moore
     *         neighbors.
     * @throws Exception If sizeFactor constraints are violated.
     */
    static List<Loc> Moore(int i, int j, int sizeFactor, int width, int height, boolean isModular)
            throws Exception {

        List<Loc> ret = new ArrayList<>();
